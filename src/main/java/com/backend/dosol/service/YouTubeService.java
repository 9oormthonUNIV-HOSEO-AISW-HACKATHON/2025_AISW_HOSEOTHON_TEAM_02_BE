...
public class YouTubeService {
    ...
    public YouTubeSearchResponse searchVideo(String title, String artist) {
        ...
        try {
            ...
            if (response != null && response.has("items") && response.get("items").size() > 0) {
                ...
                String thumbnailUrl = firstItem.get("snippet").get("thumbnails").get("default").get("url").asText();
    ...
    public void updateSongWithYoutubeInfo(Song song) {
        try {
            ...
            song.updateYoutubeInfo(response.getVideoId(), response.getYoutubeUrl());
            songRepository.save(song);
            log.info("Song YouTube 정보 업데이트 완료: songId={}, videoId={}", song.getId(), response.getVideoId());
        } catch (Exception e) {
